"""Generates the Mahnung PDF sent (or printed) for one person, covering
every billing run item included in that Mahnung.
...
parse_qrr_reference`), so a Mahnung must keep reusing each item's own,
...
The "consolidation" the Debitoren plan describes is about the *letter*
...
def generate_mahnung_pdf(
    ...
    """Render one Mahnung PDF covering every item in `candidate`.

    Args:
        connection: Open SQLite connection.
        candidate: The `app.domain.mahnwesen.MahnKandidat` to render.
        rendered_body: The Mahnung body text, placeholders already
            substituted (see `app.domain.mahnwesen.send_mahnung`),
    ...
    for item in candidate.items:
        ...
        if remaining_rappen == 0:
            ...
            # Mahnung candidate at all, since the person-level Saldo gate
            # in `app.domain.mahnwesen.list_faellige_mahnungen` would
            # normally already exclude it; this only guards the case where
            # other open items on the same person keep the Saldo positive).
            continue
        ...
        y = draw_title(canvas, f"{candidate.stufe}. Mahnung")
