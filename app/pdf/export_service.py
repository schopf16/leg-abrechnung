...
creates one combined billing document per participant plus a payment
list, all collected in `output/<year>_Q<quarter>/` (project brief,
section 6: "Alle Dokumente eines Laufs gesammelt exportierbar").
"""
...
class ExportResult:
    ...
    Attributes:
        ...
        document_paths: Paths of all generated participant billing PDFs.
        payment_list_path: Path of the generated payment list PDF, or
            `None` if the run has no payouts to make.
...
def export_billing_run_documents(
    ...
    """Generate every participant's billing document and the payment list.
    ...
    distribution = compute_quarter_distribution(connection, run.period_year, run.period_quarter)
    ...
    for item in items:
        ...
        participant_result = distribution.participant_results.get(item.participant_id)
        if participant is None or participant_result is None:
            result.errors.append(
                f"Teilnehmer #{item.participant_id} nicht gefunden (Beleg #{item.id} übersprungen)."
            )
            continue

        filename = f"Abrechnung_{_sanitize_filename_part(participant.name)}_{item.id}.pdf"
        path = output_dir / filename

        try:
            generate_participant_bill_pdf(
                run, item, participant_result, participant, settings, path
            )
