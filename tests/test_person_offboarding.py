...
def test_start_for_person_creates_tracker_with_grund_and_step_1_date(db):
    ...
    offboarding = person_offboarding_repo.start_for_person(
        db, person_id, reason="voluntary", decided_at=started
    )

    assert offboarding.id is not None
    assert offboarding.person_id == person_id
    assert offboarding.reason == "voluntary"
...
def test_start_for_person_without_date_leaves_step_1_open(db):
    person_id = _person(db)
    offboarding = person_offboarding_repo.start_for_person(db, person_id, reason="payment_default")
...
def test_start_for_person_is_idempotent(db):
    person_id = _person(db)
    first = person_offboarding_repo.start_for_person(db, person_id, reason="voluntary", decided_at=date.today())
    second = person_offboarding_repo.start_for_person(
        db, person_id, reason="payment_default", decided_at=date.today() - timedelta(days=99)
    )
    ...
    assert second.reason == "voluntary"
...
def test_current_step_since_falls_back_to_created_at_for_first_step(db):
    person_id = _person(db)
    offboarding = person_offboarding_repo.start_for_person(db, person_id, reason="voluntary")
...
def test_days_open_across_states(db):
    person_id = _person(db)
    offboarding = person_offboarding_repo.start_for_person(
        db, person_id, reason="voluntary", decided_at=date.today() - timedelta(days=10)
    )
...
def test_update_persists_all_steps(db):
    person_id = _person(db)
    offboarding = person_offboarding_repo.start_for_person(
        db, person_id, reason="payment_default", decided_at=date.today()
    )
...
def test_is_complete_once_all_four_steps_set(db):
    person_id = _person(db)
    offboarding = person_offboarding_repo.start_for_person(
        db, person_id, reason="voluntary", decided_at=date.today()
    )
    offboarding.metering_point_exit_at = date.today()
    offboarding.bkw_informed_at = date.today()
...
def test_list_in_progress_excludes_completed_trackers(db):
    ...
    person_offboarding_repo.start_for_person(db, in_progress_id, reason="voluntary", decided_at=date.today())

    done_id = _person(db, "Done")
    done = person_offboarding_repo.start_for_person(db, done_id, reason="voluntary", decided_at=date.today())
